...
class BaseFilter(BaseNode):
    def _apply_filter(self, vf_filter: str) -> bool:
        ...
        cmd = [FFMPEG_PATH, "-y", "-i", input_file, "-vf", vf_filter, output_file]
...
class WatermarkFilter(BaseFilter):
    ...
    def execute(self) -> bool:
        ...
        if not output_file:
            from pathlib import Path
            stem = Path(input_file).stem
            output_file = str(Path(input_file).parent / f"{stem}_watermark{Path(input_file).suffix}")

        cmd = [FFMPEG_PATH, "-y", "-i", input_file, "-i", watermark, "-vf", vf, output_file]
        self.log(f"Adding watermark from: {watermark}")
        code, _, _ = self.run_command(cmd, timeout=3600)
        if code == 0:
            self.job.variables["s_output_file"] = output_file
            self.job.input_file = output_file
            self.log(f"Watermark applied: {output_file}")
            return True
        self.log("Watermark failed")
        return False
...
class TimecodeFilter(BaseFilter):
    ...
    def execute(self) -> bool:
        ...
        vf = f"drawtext=text='{tc_text}':fontsize={fontsize}:fontcolor={fontcolor}:x={pos}"
        return self._apply_filter(vf)
...
class FPSFilter(BaseFilter):
    ...
    def execute(self) -> bool:
        ...
        vf = f"minterpolate=fps={target_fps}:mi_mode={algo}"
        return self._apply_filter(vf)
